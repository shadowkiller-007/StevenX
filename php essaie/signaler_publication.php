    ...
    if ($deja_signale) {
        ...
        if (empty($motif)) {
            ...
            try {
                ...
                $stmt = $pdo->prepare("INSERT INTO signalements (user_id, type, element_id, raison) VALUES (?, 'publication', ?, ?)");
                $stmt->execute([$_SESSION['user_id'], $publication_id, $raison_complete]);
                ...
                if ($nb_signalements >= 5) {
                    ...
                    $stmt->execute([$publication['user_id'], $message, $publication_id]);
                    
                    $success = true;
                    $message_succes = "Publication signalée et supprimée (5 signalements atteints)";
                } else {
                    // Notifier l'auteur du signalement
                    if ($publication['user_id'] != $_SESSION['user_id']) {
                        $message = "Votre publication a été signalée par un utilisateur pour : " . $motif;
                        $stmt = $pdo->prepare("INSERT INTO notifications (user_id, type, message, element_id) VALUES (?, 'signalement', ?, ?)");
                        $stmt->execute([$publication['user_id'], $message, $publication_id]);
                    }
                    
                    $success = true;
                    $message_succes = "Publication signalée avec succès";
                }
...
<head>
    ...
    <title>Signaler une publication</title>
            ...
            <div class="col-md-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="text-center">
                            <a href="dash.php" class="btn btn-secondary">Retour à l'accueil</a>
                        </div>
                        <?php else: ?>
                        <!-- Aperçu de la publication -->
                        <div class="alert alert-info">
                            <h6>Publication à signaler :</h6>
                            <div class="border p-3 mt-2 bg-light">
                                <strong>Par :</strong>
                                <?php echo htmlspecialchars($publication['prenom'] . ' ' . $publication['nom']); ?><br>
                                <strong>Contenu :</strong><br>
                                <?php echo nl2br(htmlspecialchars(substr($publication['contenu'], 0, 200))); ?>
                                <?php if (strlen($publication['contenu']) > 200): ?>...<?php endif; ?>
                            </div>
                        </div>
                        ...
                        <form method="POST">
                            <div class="mb-3">
                                ...
                                <select class="form-control" id="motif" name="motif" required>
                                    ...
                                    <option value="Violation des droits d'auteur"
                                        <?php echo (isset($_POST['motif']) && $_POST['motif'] == 'Violation des droits d\'auteur') ? 'selected' : ''; ?>>
                                        Violation des droits d'auteur</option>
                                    <option value="Autre"
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="raison_details" name="raison_details" rows="3"
                                    placeholder="Expliquez en quelques mots pourquoi vous signalez cette publication..."><?php echo isset($_POST['raison_details']) ? htmlspecialchars($_POST['raison_details']) : ''; ?></textarea>
                            ...
                            <div class="alert alert-warning">
                                ...
                                <ul class="mb-0">
                                    ...
                                    <li>Cette publication sera supprimée automatiquement après 5 signalements</li>
